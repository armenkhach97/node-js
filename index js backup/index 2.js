...
const getData = async () => {
	...
	console.log(productswifishopean);
	...
	for (const product of products) {
		if (
			...
			if (productswifishopean.includes(product.ean) ||
				productswifishopean.includes(parseInt(product.ean))){
				continue;
			}
			...
			const updateTable = fs
					...
					.split("\n");
			for (let i = 1; i < updateTable.length; i++) {
				const productUPDATE = updateTable[i];
	...
	for (let i = 1; i < csvTable3.length; i++) {
		...
		if (productswifishopean.includes(productDetails[1]) ||
			productswifishopean.includes(parseInt(productDetails[1]))){
			continue;
		}
		let shippingtime = "3Tage";
		const updateTable = fs
	...
	for (let i = 1; i < table4.length; i++) {
		...
		if (productswifishopean.includes(productDetails[3]) ||
			productswifishopean.includes(parseInt(productDetails[1]))){
			continue;
		}
	...
	for (let i = 1; i < table5.length; i++) {
		...
		if (productswifishopean.includes(productDetails[5]) ||
			productswifishopean.includes(parseInt(productDetails[5]))){
			continue;
		}
		let shippingtime = "3Tage";
		const updateTable = fs
